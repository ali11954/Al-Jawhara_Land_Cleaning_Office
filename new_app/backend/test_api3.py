...
with app.test_client() as client:
    ...
    if data.get('success'):
        ...
        print(f'Dashboard: {json.dumps(dash_data.get("data", {}), indent=2, ensure_ascii=False)}')
    ...
    # Test login as supervisor jaber (company 3)
    response = client.post('/api/auth/login', 
        json={'username': 'jaber', 'password': '123456'},
        content_type='application/json')
    data = response.get_json()
    print('\n=== Login as jaber (supervisor, company 3) ===')
    print(f'Success: {data.get("success")}')
    if data.get('success'):
        token = data['data']['token']
        headers = {'Authorization': f'Bearer {token}'}
        ...
        emp_data = response.get_json()
        emps = emp_data.get('data', [])
        print(f'Employees visible to jaber: {len(emps)}')
        company_ids = set(e.get('company_id') for e in emps)
        print(f'Company IDs: {company_ids}')

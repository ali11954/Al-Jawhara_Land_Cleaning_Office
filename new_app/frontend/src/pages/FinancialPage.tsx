...
export default function FinancialPage() {
  ...
  const loadData = () => {
    Promise.all([api.get('/financial/transactions'), api.get('/financial/dashboard'), api.get('/employees?is_active=true'), api.get('/accounts/balance'), api.get('/suppliers')])
  ...
  const printVoucher = (tx: any) => {
    ...
    win.document.write(`<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>سند صرف</title>
      ...
      <div class="header"><h1>سند صرف نقدي</h1><h2>ارض الجوهرة لخدمات النظافة</h2></div>

...
import {
  User, Mail, Phone, Lock, Shield, Calendar, Clock, Edit3,
  Save, Loader2, Eye, EyeOff, Briefcase, Building2, DollarSign,
  CheckCircle2, AlertCircle, BarChart3, TrendingUp, Award, CalendarDays,
  BadgeCheck, Key
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              ...
              {editMode && (
                <div className="flex justify-end gap-3 pt-2">
                  <button
                    onClick={() => { setEditMode(false); setEditForm({ full_name: profile.full_name, email: '', phone: '' }); }}
        ...
        {/* Right Column - Stats & Employee Info */}
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                  ...
                  <span className="text-sm font-medium text-gray-900">{stats.employee.position || '—'}</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                ...
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                  ...
                  <span className="text-sm font-medium text-gray-900">{(stats.employee.salary || 0).toLocaleString()} ر.س</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                  <span className="text-sm text-gray-500">الراتب الإجمالي</span>
                  <span className="text-sm font-medium text-primary-600">{(stats.employee.total_salary || 0).toLocaleString()} ر.س</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                  <span className="text-sm text-gray-500">البدل اليومي</span>
                  <span className="text-sm font-medium text-gray-900">{(stats.employee.daily_allowance || 0).toLocaleString()} ر.س</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                ...
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                  ...
                  <span className="text-sm font-medium text-gray-900">{stats.employee.is_resident ? 'resident' : 'غير مقيم'}</span>
                </div>
                {stats.employee.hire_date && (
                  <div className="flex items-center justify-between py-2">
                    <span className="text-sm text-gray-500">تاريخ التعيين</span>
                    <span className="text-sm font-medium text-gray-900">{formatDate(stats.employee.hire_date)}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {stats?.evaluations && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                  <span className="text-sm text-gray-500">آخر تقييم</span>
                  <span className="text-lg font-bold text-primary-600">{stats.evaluations.latest_score ?? '—'}</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-gray-50">
                  <span className="text-sm text-gray-500">المتوسط</span>
                  <span className="text-sm font-bold text-gray-900">{stats.evaluations.average_score}</span>
                </div>
                <div className="flex items-center justify-between py-2">
                  <span className="text-sm text-gray-500">عدد التقييمات</span>
                  <span className="text-sm font-medium text-gray-900">{stats.evaluations.total_evaluations}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="bg-gradient-to-br from-primary-500 to-blue-600 rounded-2xl shadow-sm p-6 text-white">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between py-2 border-b border-white/20">
                  ...
                  <span className="text-lg font-bold">{(stats.salary.total_paid || 0).toLocaleString()} ر.س</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-white/20">
                  <span className="text-sm text-white/70">الراتب الأساسي</span>
                  <span className="text-sm font-medium">{(stats.salary.base_salary || 0).toLocaleString()} ر.س</span>
                </div>
                <div className="flex items-center justify-between py-2">
                  <span className="text-sm text-white/70">الراتب الإجمالي</span>
                  <span className="text-sm font-medium">{(stats.salary.total_salary || 0).toLocaleString()} ر.س</span>
                </div>
              </div>
            </div>
          )}
